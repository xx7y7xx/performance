...
def sub_code_block(code) :
	...
	return code
def sub_code_block2(s, br=config.EOL) :
    # 大括号之间存在换行符。
    pattern = "^{{{(.|" + br + ")*?^}}}$"
    re_object = re.compile(pattern, re.MULTILINE)
    return re.sub(re_object, "", s)
def sub_code_block3(s):
    pattern = r"(?P<inlinecode>!?%s(?P<inline>.*?)%s)" % (r"\{\{\{", r"\}\}\}")
    re_object = re.compile(pattern, re.UNICODE)
    return re.sub(re_object, "", s)
